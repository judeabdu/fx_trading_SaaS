  ...
  useEffect(() => {
    ...
    if (token) {
      connectDerivSocket(token, (data) => {
        ...
        // FIXED: Optional Chaining and safe checking against Deriv data structures
        if (data.msg_type === "balance" && data.balance) {
          const rawBalance = data.balance.balance ?? data.balance;
          const rawCurrency = data.balance.currency ?? "USD";
          
          setBalance(typeof rawBalance === "number" ? rawBalance : parseFloat(rawBalance || 0));
      ...
      <div style={containerStyle}>
        ...
        <div style={statsGrid}>
          <StatCard
            ...
            value={`${currency} ${Number(balance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          />
          <StatCard
