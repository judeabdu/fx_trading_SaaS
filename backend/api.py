...
# =========================
# =========================
# START BOT (DATABASE SWITCH)
# =========================
@app.post("/start-bot")
def start_bot(db: Session = Depends(get_db)):
    # Find your broker account record
    account = db.query(BrokerAccount).first()
    if account:
        account.is_active = True
        db.commit()
        return {"message": "Bot activated in database"}
    raise HTTPException(status_code=404, detail="Broker account not configured")

# =========================
# STOP BOT (DATABASE SWITCH)
# =========================
@app.post("/stop-bot")
def stop_bot(db: Session = Depends(get_db)):
    ...
    if account:
        ...
        db.commit()
        return {"message": "Bot deactivated in database"}
    raise HTTPException(status_code=404, detail="Broker account not configured")
# =========================
# BOT STATUS
# =========================
